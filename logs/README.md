...
**Several commits with random past dates** (each commit adds one line to `daily.md`; author/committer date is random within the last year, or override with `DAILY_LOG_RANDOM_DAYS`):

```bash
./scripts/daily_log.sh --random-dated-commits          # random count between 2 and 6
./scripts/daily_log.sh --random-dated-commits 8      # exactly 8 commits
./scripts/daily_log.sh --random-dated-commits 4 "note applied to every line"
```

Then `git push` when you want those commits on the remote. Requires `python3` and normal `git` user identity.
