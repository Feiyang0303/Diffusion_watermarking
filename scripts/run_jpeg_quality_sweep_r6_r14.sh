#!/usr/bin/env bash
# JPEG quality sweep for r=6 and r=14 with min_dist, k=1.0.
# Extends the main sweep (r∈{8,10,12}) to test wider/narrower Fourier masks.
#
# Usage:
#   NUM_SAMPLES=50 bash scripts/run_jpeg_quality_sweep_r6_r14.sh
...
echo "JPEG quality sweep (min_dist, extended radii)"
echo "  Q ∈ {$QUALITIES}"
echo "  R ∈ {$RADII}"
echo "  n = $NUM_SAMPLES"
